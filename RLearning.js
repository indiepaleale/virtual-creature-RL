import * as ReImprove from "reimprovejs/dist/reimprove.js";
...
export default async function step(time) {

    let inputs = new Uint8Array(64);          // Need to give a number[] of your inputs for one teacher.
    const result = await academy.step([               // Let the magic operate ...
        { teacherName: teacher, agentsInput: inputs }
    ]);
    // step(time); // Don't forget to call the loop again
    console.log(result.get(agent)); // Get the result of the agent
    
}

step(0); // Start the loop
